...
export default function ExamTracking() {
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            ...
            case "SampleCollected": return <Radiation size={16} />; // Exam Completed
            case "ResultAvailable": return <Scan size={16} />;      // Report Drafted
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case "ResultAvailable": return "bg-purple-100 text-purple-600 border-purple-200";
    ...
    const formatStatus = (status: string) => {
        if (status === "SampleCollected") return "Exam Completed";
        if (status === "ResultAvailable") return "Report Available";
        return status;
    };
        ...
        <div className="h-[calc(100vh-2rem)] flex gap-6">
            ...
            <div className="w-full max-w-md flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-slate-100 bg-slate-50/50">
                    <h2 className="text-lg font-bold text-slate-800 mb-4">Exam Tracking</h2>
            ...
            <div className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm p-8 overflow-y-auto custom-scrollbar relative">
                {!selectedOrder ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400">
                        ...
                        <p className="font-medium">Select an order to track exam status</p>
                    </div>
                ) : (
