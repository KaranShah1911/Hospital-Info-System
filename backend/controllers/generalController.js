    ...
    try {
        ...
        const visit = await prisma.opdVisit.findUnique({
            ...
            include: {
                ...
                // OPD Orders (Note: This might include IPD ones if linked to both, we'll filter)
                serviceOrders: {
                    include: {
                        service: true,
                        labResults: true,
                         // invoiceItems: true // Not critical for narrative
                    },
                    orderBy: { orderDate: 'asc' }
                },
                prescriptions: {
        ...
        // 2. Segregate Data (OPD vs IPD)
        
        // IPD Order IDs (Set for fast lookup)
        const ipdOrderIds = new Set(visit.admission?.serviceOrders.map(o => o.id) || []);
        
        // OPD Orders: Those in Visit List BUT NOT in Admission List
        const opdOrders = visit.serviceOrders.filter(o => !ipdOrderIds.has(o.id));
        
        // IPD Orders: Directly from Admission
        const ipdOrders = visit.admission?.serviceOrders || [];


        // 3. Construct the Narrative Report
        const kundali = {
            metadata: {
                reportGeneratedAt: new Date(),
                hospitalName: "MediFlow General Hospital"
            ...
            visitJourney: {
                ...
                stage2_triage: {
                    status: visit.status,
                    triageColor: visit.triageColor,
                    vitals: visit.clinicalNotes.find(n => n.noteType === 'ProgressNote' || (n.content && n.content.vitals))?.content || "Not Recorded"
                },
                stage3_consultation: {
                    notes: visit.clinicalNotes.filter(n => n.noteType === 'SOAP').map(n => ({
                        date: n.createdAt,
                ...
                stage4_diagnostics_and_medicines_OPD: {
                    orders: opdOrders.map(o => ({
                        service: o.service.name,
                        status: o.status,
                        result: o.labResults[0]?.resultValue || "Pending"
                    })),
        ...
        // 4. Add IPD Data if exists
        if (visit.admission) {
            kundali.visitJourney.stage5_hospitalization = {
                status: "Admitted",
                admissionDate: visit.admission.admissionDate,
                department: visit.admission.department.name,
                ipdDiagnostics: ipdOrders.map(o => ({
                     service: o.service.name,
                     status: o.status,
                     result: o.labResults[0]?.resultValue || "Pending"
                })),
